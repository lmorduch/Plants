...
// ABOUTME: Invoked via a temporary token-guarded admin endpoint, then removed.
import mysql from 'mysql2/promise';
import pool from './db.js';

export async function migrateMysqlToPg() {
  ...
  async function copy(table, columns, mapRow) {
    ...
    summary[table] = { read: rows.length, inserted };
  }

  try {
    // anthropic_key/hint are intentionally skipped — the app moved to Gemini,
