...
router.post('/', upload.single('photo'), async (req, res) => {
  const { name, species, location, acquired_date, notes, sun_preference, fun_facts, history_and_origin, extra_notes } = req.body;
  ...
  const [result] = await pool.execute(
    'INSERT INTO plants (user_id, name, species, location, acquired_date, notes, photo_url, sun_preference, fun_facts, history_and_origin, extra_notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [req.userId, name, species || null, location || null, acquired_date || null, notes || null, photo_url, sun_preference || null, funFactsJson, history_and_origin || null, extra_notes || null]
  );
...
router.put('/:id', upload.single('photo'), async (req, res) => {
  ...
  await pool.execute(
    'UPDATE plants SET name=?, species=?, location=?, acquired_date=?, notes=?, photo_url=?, sun_preference=?, fun_facts=?, history_and_origin=?, extra_notes=? WHERE id=? AND user_id=?',
    [name, species || null, location || null, acquired_date || null, notes || null, photo_url, sun_preference || null, funFactsJson, history_and_origin || existing.history_and_origin || null, extra_notes || existing.extra_notes || null, req.params.id, req.userId]
  );
