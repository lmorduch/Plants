...
router.post('/', upload.single('photo'), async (req, res) => {
  ...
  const [result] = await pool.execute(
    'INSERT INTO plants (user_id, name, species, location, acquired_date, notes, photo_url, sun_preference, fun_facts, history_and_origin, extra_notes, pet_safe, pet_safety_notes, life_cycle) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
router.put('/:id', upload.single('photo'), async (req, res) => {
  ...
  await pool.execute(
    'UPDATE plants SET name=?, species=?, location=?, acquired_date=?, notes=?, photo_url=?, sun_preference=?, fun_facts=?, history_and_origin=?, extra_notes=?, pet_safe=?, pet_safety_notes=?, life_cycle=? WHERE id=? AND user_id=?',
    [name, species || null, location || null, acquired_date || null, notes || null, photo_url, sun_preference || null, funFactsJson, history_and_origin || existing.history_and_origin || null, extra_notes || existing.extra_notes || null, pet_safe || existing.pet_safe || null, pet_safety_notes || existing.pet_safety_notes || null, life_cycle || existing.life_cycle || null, req.params.id, req.userId]
  );
